...
#include "../build/minter_set_erc721_unwrapping_fees.mligo"

let set_erc721_unwrapping_fees  = 
    fun (u:unit) -> 
        match ((Tezos.get_entrypoint_opt "%set_erc721_unwrapping_fees" contract_address): nat contract option) with 
        | Some v ->
            ([Tezos.transaction new_erc721_unwrapping_fees 0tez v]: operation list)
        | None -> (failwith "not_found": operation list) 
    

let minter_set_erc721_unwrapping_fees_payload = 
    let l = Operation set_erc721_unwrapping_fees in
