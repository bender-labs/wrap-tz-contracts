...
#include "../build/governance_confirm_oracle_migration.mligo"

let confirm_migration  = 
    fun (u:unit) -> 
        match ((Tezos.get_entrypoint_opt "%confirm_oracle_migration" contract_address): unit contract option) with 
        | Some v ->
            ([Tezos.transaction () 0tez v]: operation list)
        | None -> (failwith "not_found": operation list) 
    

let governance_confirm_oracle_migration_payload = 
