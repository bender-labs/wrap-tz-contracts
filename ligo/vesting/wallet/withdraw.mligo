...
let withdrawal_fees (level, amnt, s: nat * nat * storage): nat * nat =
    let cycles = sub(Tezos.level, level) / s.settings.blocks_per_cycle in
    let fee = 
...
let withdraw(stake_index, amnt, s : nat * nat * storage) : (operation list) * storage = 
    ...
    let op = transfer_multiple (Tezos.self_address, s.settings.staked_token,[ (Tezos.sender, withdraw_amount) ; (s.settings.burn_address, burn)]) in
