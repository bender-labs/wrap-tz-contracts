...
let transfer_multiple ((from_, token, txs):(address * token * (address * nat ) list)): operation = 
    let (addr, token_id) = token in
    let transfer = {
        from_ = from_;
        txs = List.map (fun (to_, amnt:address * nat) -> {to_ = to_ ; amount = amnt ; token_id = token_id} ) txs
    } in
    let ep : ((transfer list) contract) option = Tezos.get_entrypoint_opt "%transfer" addr in
    match ep with
    | Some v -> Tezos.transaction [transfer] 0tez v
