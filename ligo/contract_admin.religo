...
let fail_if_not_admin = (s:contract_admin_storage) : unit => {
  if(s.administrator != Tezos.sender) {
    failwith("NOT_ADMIN");
  };
};

let fail_if_not_signer = (s:contract_admin_storage) : unit => {
  if(s.signer != Tezos.sender) {
    failwith("NOT_SIGNER");
  };
};

let fail_if_not_governance = (s:contract_admin_storage) : unit => {
